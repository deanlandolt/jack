...
var chomp = function(string, separator) {
    var extra = separator ? separator + "|" : "";
    return string.replace(new RegExp("(" + extra + "\\r|\\n|\\r\\n)*$"), "");
};
...
var URLMap = exports.URLMap = function(map, options) {
    ...
    for (location in map) {
        ...
        mapping.push([host, chomp(location, "/"), app]);
    }
    ...
    return function(request) {
        var path  = request.pathInfo ? request.pathInfo.squeeze("/") : "",
            hostHeader = request.headers.host, reqHost = request.host, port = request.port;

        for (var i = 0; i < mapping.length; i++)
        {
            var host = mapping[i][0], location = mapping[i][1], app = mapping[i][2];

            if ((host === hostHeader || host === reqHost || (host === null && (hostHeader === reqHost || hostHeader === reqHost + ":" + port))) &&
